...
def glyph_dims(fontpack, data: bytes):
    """{font_index: [(w,h), ...]} — for render-drift diagnosis. Uses parse_pack's
    font table + a light glyph-record walk (8B/glyph: off,w,h,xadv,xoff,yoff)."""
    import struct
    ...
    for fi, f in enumerate(p.fonts):
        ...
        dims = []
        for gi in range(n):
            base = f.glyph_off + gi * 8
            _off, w, h = struct.unpack_from("<HBB", data, base)
            dims.append((w, h))
        out[fi] = dims
    return out
...
def main():
    ...
    # also build a baseline at the CURRENT versions to isolate glyph drift vs ver byte
    ...
    for b in bundles:
        ...
        if shipped is None:
            ...
            same_ver = base_by_id[bid]  # this bundle rebuilt at the CURRENT version
            if same_ver != shipped:
    ...
    if not args.apply:
        ...
        # render-drift hint: for a glyph-changed bundle, show a couple of WxH diffs
