...
- **Ask before posting anything to GitHub, and leave the Claude Code footer off
  comments.** Draft every issue comment, PR body and review reply in chat first and
  post only on an explicit go-ahead — these go out under the owner's account and are
  the project's public voice, not ours. What got edited out of a real reply
  (PolyKybdHost issue #131, 2026-08-02): a *"one caveat that matters for you"*
  paragraph, a sentence introducing a table that spoke for itself, and hedging
  (*"still not scheduled, but…"* where *"in progress"* was the truth). **Shorter,
  fewer caveats, less hedging** — the instinct to pre-empt every misreading reads as
  noise to the person who filed the issue. A follow-up comment correcting my own
  wording was deleted as redundant: ask rather than stack a correction on a
  correction. The `_Generated by Claude Code_` footer is **not** added to new
  comments (ones that already carry it keep it).
