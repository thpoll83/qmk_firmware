...
void raw_hid_receive(uint8_t *data, uint8_t length) {
    ...
    if(data[0] == id_custom_save || data[0] == 'P') {
        ...
        switch(data[1]) {
            ...
            // ------------------------------------------------------------------
            // HID firmware update commands (0x40 – 0x43) — handled in hid_fw_up.c
            // ------------------------------------------------------------------
            case CMD_FW_UP_BEGIN:
            case CMD_FW_UP_CHUNK:
            case CMD_FW_UP_COMMIT:
            case CMD_FW_UP_GET_VERSION:
                hid_fw_up_receive(data, length);
                break;
