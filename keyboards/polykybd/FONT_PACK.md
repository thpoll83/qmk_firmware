...
- **Regenerate** with `FONTCONVERT=<pinned> python3 generate_fonts.py`. **Byte-repro
  ...
  header) — if other category headers diff, the toolchain/source drifted. ⚠️ **That
  is a DRIFT CHECK on the generated headers, not the reship checklist** — shipping the
  result still needs the `.plyf`, `bundles.json` and `content_version` bump under
  *"Reshipping a bundle to the host"* above. Note the two manifests are different files:
  `fontpack.manifest.json` here, `fontpack_bundles.manifest.json` there.
